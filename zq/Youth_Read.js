...
let YouthBody = $.getdata('youth_autoread') || $.getdata("zqwzbody");
let zq_timebody = $.getdata("zq_timebody");
    ...
    if ($request.url.match(/\/kandian.youth.cn\/v5\/article\/complete.json/)) {
        ...
        if (YouthBody) {
            if (YouthBody.indexOf(bodyVal) > -1) {
                ...
                YouthBody = YouthBody + "&" + bodyVal;
                $.setdata(YouthBody, 'zqwzbody');
                $.log(`${$.name}获取阅读: 成功, zqwzbodys: ${bodyVal}`);
                bodys = YouthBody.split("&")
...
if (isGetbody = typeof $request !== `undefined`) {
    ...
    if ($.isNode()) {
        $.log("优先读取zqllzbody.txt文件，如果没有文件读取环境变量YOUTH_READ")
        var fs = require("fs");
        YouthBody = fs.readFileSync("zqllzbody.txt", "utf8");
    }
    ...
    function batHost(api, body) {
        return {
            ...
            headers: {
                'os-version': ' RKQ1.200826.002+test-keys',
                'device-model': ' Mi+10',
                'phone-sim': ' 1',
                'carrier': ' %E4%B8%AD%E5%9B%BD%E7%A7%BB%E5%8A%A8',
                'request_time': ' 1637142241',
                'access': ' WIFI',
                'iid': ' 0',
                'os-api': ' 26',
                'device-platform': ' android',
                'openudid': ' e3644676f93836bb',
                'app-version': ' 2.4.3',
