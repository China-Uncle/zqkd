...
#中青看点每日收益查询
https://kandian.youth.cn/v17/NewTask/getTaskList.json 重写目标 https://raw.githubusercontent.com/shaolin-kongfu/js_scripts/main/zq/zq_today_score.js
[MITM]
...
function nickname(zq_cookie2,timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url : 'https://kandian.youth.cn/v17/NewTask/getSign.json?'+ zq_cookie2,
            headers : {
...
function today_score(zq_cookie1,nickname1,timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url : 'https://kandian.youth.cn/wap/user/balance?'+ zq_cookie1,
            headers : {
    'Host': 'kandian.youth.cn'
},
            }
...
async function getzq_cookie() {
    if ($request.url.match(/\/kandian.youth.cn\/v17\/NewTask\/getTaskList/)) {
